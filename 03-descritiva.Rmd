...
## Dados brutos e pr-e-processamentos elementares
...
Assim, para a determinação das **posições virtuais** dos 3 quartis pela **primeira regra** o valor de _p_ seria:

\


- para o _primeiro quartil_ ($L_{q_{0,25}}=0,25 \times (n)$; 
- para o _segundo quartil_ ($L_{q_{0,50}}=0,50 \times (n)$; ou,  
...
E as aproximações dos quartis serão:
...
| Potência de 2              | 25  $\leq$ n $\leq$ 220  | $2^{k} > n$             |
...
Primeiramente identificamos a **classe mediana**: a classe que contem o elemento de posição $\frac{n}{2}$ (basta observar a coluna da frequência absoluta acumulada: $F_{i}$, percorrendo-a até a classe $i$ que tenha valor $>\frac{n}{2}$. Os demais elementos da expressão da mediana são:   
...
- $\Delta_{i}$: é o intervalo da **classe mediana**.
...
## Apresentacao descritiva de dados na forma grafica
