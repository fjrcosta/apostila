...
# Introdução às estatísticas epidemiológicas{#introducao-as-estatisticas-epidemiologicas}
...
\hline
\multirow{2}{*}{Fator estudado} & \multicolumn{2}{c|}{Desfecho observado} & Total \\ 
 & \cline{2-3}   
              Presente  & Ausente       \\ 
\hline              
Exposto & (a) &  (b)   & (e) \\
...
\begin{tabular}{c|c|c|c}
\hline
\multirow{2}{*}{Fator estudado: hábito da mãe} & \multicolumn{2}{c|}{Desfecho observado: baixo peso ao nascer} & \multirow{2}{*}{Total}  \\ 
& \cline{2-3}
              Sim & Não        \\ 
\hline              
...
$$
I_{e}=\frac{\text{expostos com desfecho}}{\text{total de expostos}}\\
I_{e}=\frac{(a)}{(e)} \times 100  \\
I_{e}= \frac{275}{2.419} \times 100 = 11,37 \%
$$
\

>Interpretação: 11,37\% das crianças de mães tabagistas no estudo nasceram com baixo peso.
...
$$
I_{0}=\frac{\text{não expostos com desfecho}}{\text{total de não expostos}}\\
I_{0}= \frac{(c)}{(f)} \times 100 \\
I_{0}= \frac{311}{4.807} \times 100 = 6,47 \%
$$

\

>Interpretação: 6.47\% das crianças de mães não tabagistas no estudo nasceram com baixo peso.
...
$$
P= \frac{\text{total com desfecho}}{\text{total geral}}\\
P= \frac{(a) + (c)}{(e) + (f)} \times 100 \\
P= \frac{586}{7.226} \times 100 =  8,11\%
$$

\

>Interpretação: 8,11\% das crianças no estudo nascera com baixo peso.
...
$$
RA=I_{e} - I_{0}  \\
RA= 0,11368334 - 0,064697316=0,049 \\
RA= 4,9 \%
$$

\

>Interpretação: 4,9\% do risco de uma criança nascer com baixo peso pode ser atribuído ao fato de terem mães tabagistas.
...
- razão de risco maior que 1: diz-se então que o fator estudado é um **fator de risco** em relação ao desfecho observado;  
- razão de risco menor que 1: diz-se então que o fator estudado é um **fator protetor** em relação ao desfecho observado.  
...
$$
RR =  \frac{I_{e}}{I_{0}}  \\
RR = \frac{0,11368334}{0,064697316} \\
RR = 1,76 
$$

\

>Interpretação: O fator estudado (hábito das mães) é um fator de risco para o desfecho observado (nascimento com com baixo peso). As crianças de mães tabagistas têm aproximadamente 1,76 vezes mais risco de nascer com baixo peso em comparação com as de mães não tabagistas. 
...
$$
FE = \frac{I_{e}-I_{0}}{I_{e}} \times 100
$$
...
- $I_{0}$: é a incidência da doença no grupo não exposto. 
...
$$
FE = \frac{I_{e}-I_{0}}{I_{e}} \times 100\\
FE=  \frac{\left(0,11368334 - 0,064697316\right)}{0,11368334}=0,4309 \\
FE= 43,09 \%
$$
...
## _Odds ratio_ (Razão das chances) em estudos de casos e controles
