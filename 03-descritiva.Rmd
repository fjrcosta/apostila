...
# Introdução à estatística descritiva{#introducao-a-estatistica-descritiva}
...
<iframe src="assets/descritiva.html" width="100%" height="500px" style="border:none;"></iframe>
...
## Dados brutos e pré-processamentos elementares
...
>_Garbage in_, _garbage out_. 


\

Não são raras as vezes nas quais o relatório com os dados coletados em uma pesquisa apresentam uma série de erros. Não estamos a nos refeir aqui aos **erros amostrais** mas sim aos erros experimentais (não amostrais), aqueles decorrentes de dados coletados incorretamente, tais como aqueles resultantes de omissões na transcrição das informações, da leitura de instrumentos descalibrados ou de informações simplesmente não coletadas.
...
## Apresentacao descritiva de dados na forma de resumos numericos
...
- **dispersão (variabilidade)**: 
  - absolutas (amplitude total, variância e desvio padrão)
  - relativas (coeficiente de variação, unidades padronizadas); e,
- **subdivisão (separatrizes, quantis)**: mediana (50\%), quartis (25\%, 50\%, 75\%), decis (10\%, ....90\%) e percentis (1\%....99\%).

\

Uma  medida de posição ou dispersão é dita **resistente** quando forem **pouco afetadas** pela alteração de uma pequena porção dos dados. 

A mediana é uma medida resistente, já a média e a variância não são.
...
```{r, echo=FALSE, out.width='50%', fig.align='center', fig.cap="Viés e variância"}

knitr::include_graphics("images3/bias_variance.jpg")
...
```{r, echo=FALSE, out.width='80%', fig.align='center', fig.cap="Mudanças na média pela adição (subtração) de uma constante $k=0.05$"}
...
```{r, echo=FALSE,  out.width='80%', fig.align='center', fig.cap="Mudanças na média pela multiplicação (divisão) de uma constante $k=1.2$"}
...
print(paste("A média dos 60 estudantes é:", round(mean(alturas), 4)))
...
```{r,echo=FALSE,align='center', fig.cap="Bimodal: 1,66 m e 1,73 m"}
...
O conhecimento de uma medida de tendência central nos provê uma informação útil mas incompleta. 

\

As medidas de dispersão nos ajudam a ter uma perspectiva melhor dos dados e podem ser de dois diferetes tipos quanto à dimensão na qual são expressas (unidade):

\

- **medidas absolutas** são aquelas que carregam a unidade de medida da variável do fenômeno estudado ($m;kg;\frac{R\$}{mês};\dots$):   
  - amplitude total dos dados; e,
  - desvio padrão.  
  
- **medidas relativas** são **adimensionais**, isto é, não carregam a unidade de medida da variável do fenômeno estudado. Assim podem ser usadas para se comparar a variabilidade de dois ou mais conjuntos de dados, mesmo quando as variáveis se refiram a diferentes fenômenos ou que sejam expressas, originalmente, em diferentes unidades.
...
```{r, echo=FALSE}
# Variância
print(paste("A variância das medidas das alturas dos 60 estudantes é:", round(var(alturas), 4)))

# Desvio padrão
print(paste("O desvio padrão das medidas das alturas dos 60 estudantes é:", round(sd(alturas), 4)))
...
- **somando-se (ou subtraindo-se)** cada um dos elementos do conjunto de dados por uma constante arbitrária, **a variância não se altera**; e,
- **multiplicando-se (ou dividindo-se)** cada um dos elementos do conjunto de dados por uma constante arbitrária, **a variância ficará multiplicada (ou dividida) pelo quadrado dessa constante**. 
- **multiplicando-se (ou dividindo-se)** cada um dos elementos do conjunto de dados por uma constante arbitrária, **o desvio padrão** fica **multiplicado (ou dividido) por essa constante**  
...
alturas_ad=alturas+0.05
# Multiplicando-se por uma constante k=1.2
alturas_mult=alturas*1.2
# Dividindo-se por uma constante k=1.2
alturas_div=alturas/1.2


print(paste("A variância das medidas das alturas dos 60 estudantes adicionada de 0.05 (k) é:", round(var(alturas_ad), 4), ", igual à variância sem a adição:" , round(var(alturas),4)))

print(paste("A variância das medidas das alturas dos 60 estudantes multiplicadas por k=1,20 é:", round(var(alturas_mult), 4), ", igual à variância original multiplicada pelo quadrado dessa constante (k):" , round((var(alturas)*(1.2^2)),4)))

print(paste("A variância das medidas das alturas dos 60 estudantes dividida por k=1,20 é:", round(var(alturas_div), 4), ", igual à variância original dividida pelo quadrado dessa constante (k):" , round((var(alturas)/(1.2^2)),4)))
...
A Tabela 1 apresenta os valores de $m$ para cada uma das nove definições
catalogadas por Hyndman e Fan (1996), com indicação da família a que pertencem.
...
O parâmetro é $m = \frac{p+1}{3}$, de modo que o índice e o peso de interpolação
são:
...
cat("\nVerificacao via quantile(..., type = 8):\n")
print(quantile(x, probs = c(0.25, 0.50, 0.75), type = 8))
...
Assim, para a determinação das **posições virtuais** dos 3 quartis os valores de _p_ seriam:

\


- para o _primeiro quartil_ ($L_{q_{0,25}}=0,25 \times (n)$; 
- para o _segundo quartil_ ($L_{q_{0,50}}=0,50 \times (n)$; e,  
- para o _terceiro quartil_ ($L_{q_{0,75}}=0,75 \times (n)$. 


\


Novamente podemos nos deparar com **duas situações possíveis** para o valor calculado para a posição $L_{p}$. No $R$ a função ``quantile'' com argumento ``type=2'' faz a estimação do quantil seguindo as regras ao se usar $L_{p}=p\times n$: 
...
Para uma distribuição **simétrica** (ou aproximadamente simétrica), deveremos
observar:
...
Em razão das variadas proposições existentes na literatura para se estabelecer os quantis o [R](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/quantile) apresenta 9 modos diferentes: 
...
for m in methods:
   ...
   print(f"Estimativas dos quartis segundo o método {m:<25} => Q1: {estimates[0]:.5f}, Q2: {estimates[1]:.5f}, Q3: {estimates[2]:.5f}")
...
| Proposição para determinação da posição virtual | Se \(L_p\) é inteiro | Se \(L_p\) é fracionário | Correspondência no R |
|:--|:--|:--|:--|:--|
...
## Padronizacao de dados (_z-scores_)
...
```{r, echo=FALSE, out.width='80%', fig.align='center', fig.cap="Diferentes formas na distribuição dos dados"}
...
A _assimetria_ é classificada do seguinte em razão do valor calculado:
...
### Diferentes posições da média, moda e mediana (2$^{o}$ quartil) em razão da forma da distribuição
...
```{r, echo=FALSE, out.width='80%', fig.align='center', fig.cap="Valores observados das alturas dos estudantes e as posições da média, moda e mediana"}
barplot(tab_alturas,
        main="Valores observados da alturas dos estudantes",
        xlab="Altura (cm)",
        ylab="Quantidade observada (un)",
...
: Quadro comparativo entre as medidas de tendência central (posição)

\

## Apresentacao descritiva de dados na forma tabular
...
- dispersão: amplitude total dos dados, variância/desvio padrão, coeficiente de variação;
...
Uma tabela é uma **forma não textual** (não textual/discursiva/) de apresentar informações nas quais o dado numérico se destaca como informação central. 
...
A explicação detalhada da análise realizada é assunto para ser exposto no corpo principal do texto ou em apêndices. 

\


Uma tabela serve para apresentar de modo sintético esses resultados assim, possui algums **elementos obrigatórios** (fora o próprio corpo tabular) para que a compreensão do leitor não se desvie muito para o texto:   

\


- **título**;
- **cabeçalho explicativo nas colunas e linhas**;
- **fonte dos dados**.

\

Os dados mostrados no corpo da tabela devem  apresentar uniformidade no número de casas decimais e, para situações adversas, símbolos que expliquem a ausência da informação: NI (não informado), NE (não existente), ou 0 (zero).


\


Trabalhos de natureza acadêmica ou científica _deveriam_ obrigatoriamente seguir, quando publicados no Brasil, a norma vigente publicada pela ABNT: Associação Brasileira de Normas Técnicas e algumas punlicações do IBGE: Instituto Brasileiro de Geografia e Estatística (como em [link](https://biblioteca.ibge.gov.br/visualizacao/livros/liv23907.pdf)).
...
### Apresentacao tabular de dados qualitativos


\

#### Tabelas de entrada única 
...
tab <- data.frame(
  `Estrutura domiciliar` = c("Casal com filho(s)", "Casal sem filhos", "Totais"),
  Democrata              = c(762, 484, 1246),
  Republicano            = c(468, 477, 945),
  Totais                 = c(1230, 961, 2191),
  check.names = FALSE
)
...
knitr::kable(tab,
  ...
  kableExtra::footnote(general = "Fonte: Próprio autor",
                       general_title = "")
...
### Apresentacao tabular de dados quantitativos
...
A _amplitude total_ (_A_) dos valores observados é a simples diferença entre o _valor máximo_ (2,00 m) e o _valor mínimo_ (1,41 m):
...
A amplitude de cada uma das classes ( $\Delta$) será dada pelo quociente da _amplitude total_ ( _A_) pelo _número de classes_ ( _k_). 
\
...
```{r, echo=FALSE, message=FALSE,out.width='80%', fig.align='center', fig.cap="Polígono de densidades relativas"}
...
```{r, echo=FALSE}
# -------------------------------------------------------
# Função: tabela_freq() com resultados tendo n. de casas da variável + 1
# Parâmetros:
#   x       : vetor numérico de dados brutos
#   k       : número de classes
#   delta   : amplitude de cada classe (igual para todas)
#   lim_inf : limite inferior da primeira classe
#   format  : "console" (padrão) ou "html"
# -------------------------------------------------------

tabela_freq <- function(x, k, delta, lim_inf, format = "console") {
  n <- length(x)
  casas_x <- max(nchar(sub("^[0-9]*\\.?", "", as.character(x[grepl("\\.", as.character(x))]))))
  casas_x <- ifelse(length(casas_x) == 0 || is.infinite(casas_x), 0, casas_x)
  casas_fri <- casas_x + 1
  casas <- nchar(sub("^[0-9]*\\.?", "", as.character(delta)))
  casas <- ifelse(grepl("\\.", as.character(delta)), casas, 0)
  limites_inf <- lim_inf + (0:(k - 1)) * delta
  limites_sup <- limites_inf + delta
  pm <- (limites_inf + limites_sup) / 2
  fi <- integer(k)
  for (i in seq_len(k)) {
    if (i < k) {
      fi[i] <- sum(x >= limites_inf[i] & x < limites_sup[i])
    } else {
      fi[i] <- sum(x >= limites_inf[i] & x <= limites_sup[i])
    }
  }
  ...
  di  <- fi  / delta
  dri <- fri / delta
  fri_arred <- round(fri * 100, 6)
  diff_fri  <- 100 - sum(fri_arred)
  if (diff_fri != 0) {
    idx <- which.max(fri_arred)
    fri_arred[idx] <- fri_arred[idx] + diff_fri
  }
  Fri_pct_arred    <- round(Fri * 100, 4)
  Fri_pct_arred[k] <- 100
  classe_label <- paste0(
    "[", format(limites_inf, nsmall = 2), " ; ",
    format(limites_sup, nsmall = 2), ")"
  )
  classe_label[k] <- sub("\\)$", "]", classe_label[k])
  tab <- data.frame(
    Classe = classe_label,
    PM     = pm,
    fi     = fi,
    fri    = round(fri, casas_fri),   # <-- casas_fri
    Fi     = Fi,
    Fri    = round(Fri, casas_fri),   # <-- casas_fri
    fi_pct = fri_arred,
    Fi_pct = Fri_pct_arred,
    di     = round(di, casas),
    dri    = round(dri, casas_fri)    # <-- casas_fri
  )
  names(tab) <- c(
    "Classe", "PM",
    "fi", "fri",
  ...
  totais <- c(
    "Total", "",
    sum(fi), round(sum(fri), casas_fri),
    "", "",
    sum(fri_arred), "",
    "", ""
  )
  tab_total <- rbind(sapply(tab, as.character), totais)
  tab_total <- as.data.frame(tab_total, stringsAsFactors = FALSE)
  names(tab_total) <- names(tab)
  rownames(tab_total) <- c(as.character(1:k), "Total")
  
  if (format == "html") {
    if (knitr::is_latex_output()) {
      knitr::kable(tab_total, format = "latex", align = "c",
                   booktabs = TRUE,
                   row.names = TRUE,
                   col.names = c("$i$", "Classe", "$\\bar{x}_i$", "$f_i$", "$f_{ri}$",
                                 "$F_i$", "$F_{ri}$", "$f_i$ (%)", "$F_i$ (%)",
                                 "$\\delta_{i}$", "$\\delta_{ri}$")) |>
        kableExtra::kable_styling(
        ...
        kableExtra::row_spec(nrow(tab_total), bold = TRUE)
    } else {
      knitr::kable(tab_total, format = "html", align = "c",
                   row.names = TRUE,
                   col.names = c("$i$", "Classe", "$\\bar{x}_i$", "$f_i$", "$f_{ri}$",
                                 "$F_i$", "$F_{ri}$", "$f_i$ (%)", "$F_i$ (%)",
                                 "$\\delta_{i}$", "$\\delta_{ri}$"),
...
tabela_freq(alturas, k = 8, delta = 0.075, lim_inf = 1.41, format = "html")
...
cat("\nÁrea total sob o polígono:", round(area_total, 6), "\n")

cores <- c("#E07070", "#90C090", "#8080D0", "#D4C060",
...
>**A classe mediana ($k_{md}$)é a classe que contém o elemento de posição $\frac{n}{2}$ (basta observar a coluna da frequência absoluta acumulada: $F_{i}$, percorrendo-a até a classe $i$ que tenha valor $>\frac{n}{2}$. Os demais elementos da expressão da mediana são:**   

\

- $l_{inf_{md}}$: limite inferior da **classe mediana**;  
...
### Variância e desvio padrão para dados tabelados
...
O desvio padrão para dados tabelados segue sendo a raiz quadrada da variância
...
### Coeficiente de variação para dados tabelados.
...
## Apresentacao descritiva de dados na forma gráfica
...
A partir das tabelas mostradas na seção 3.5.1.1 Dados qualitativos em entrada única, podemos elaborar a apresentação gráfica na forma de _Gráficos de colunas_:
 

\


```{r, echo=FALSE, out.width='80%', align='center',  fig.cap="Gráfico de barras dos dados observados no terminal de desembarque internacional do aeroporto"}
...
```{r, echo=FALSE,  align='center', fig.cap="Gráfico de barras da estrutura domiciliar dos Estados Unidos"}
...
```{r, echo=FALSE,   out.width='80%',align='center', fig.cap="Gráfico de setores dos desembarques observados no terminal de desembarque internacional do aeroporto"}
...
```{r, echo=FALSE,  align='center', fig.cap="Gráfico de setores da estrutura domiciliar dos Estados Unidos"}
...
```{r, echo=FALSE,  align='center', fig.cap="Gráfico de barras da estrutura familiar em relação à inclinação partidária nos Estados Unidos"}
...
```{r, echo=FALSE,  align='center', fig.cap="Gráfico de barras da exposição ao fator de risco e o efeito"}
...
```{r, echo=FALSE,   out.width='80%', fig.align='center' , fig.cap="Gráfico de dispersão unidimensional (stripchart)"}
...
```{r, echo=FALSE,  align='center', fig.cap="Gráfico de barras dos dados brutos: uma barra para cada observação e sua altura expressando o número de observações com esse valor"}
...
```{r, echo=FALSE,   out.width='80%', fig.align='center', fig.cap="Histograma das alturas dos estudantes com as posições da média, moda e mediana"}
...
```{r, echo=FALSE, output.width='80%',  align='center', fig.cap="A linha vermelha é uma aproximação da Função de Densidade da frequência relativa de observação (a linha preta é a curva da função densidade de uma distribuição Normal com média e variâncias dadas pelos dados"}
...
```{r, echo=FALSE, out.width='70%', fig.align='center', fig.cap="Polígono de densidades relativas como uma densidade empírica de probabilidade"}

knitr::include_graphics("images3/dens_rel.jpg")
...
```{r, echo=FALSE,  output.width='80%', align='center', fig.cap="Gráfico de setores das alturas dos estudantes"}
...
- as observações mais extremas, situadas fora dos limites _LI_ e _LS_ (são rotuladas como _outliers_).

\

Um _outlier_ é uma observação que merece atenção do pesquisador. Pode tanto representar um dado **realmente** observado (pouco provável de sê-lo, mas que na pesquisa o foi), quanto um **erro não amostral**. 

\


Roteiro para o _Box-Plot_ cf. John Tukey ( _Exploratory Data Analysis_, 1977)
...
- as observações que superaram os limites ( _outliers_ ) são identificadas com seus valores e permanecem no gráfico
- remover o tracejado dos limites

\

```{r, echo=FALSE,   output.width='80%', fig.width=9, fig.height=7, fig.align='center', fig.cap="Box-plot do conjunto de dados das alturas"}
...
text(x = 0.55, y = q2 + 0.04, "Seg. quartil-Q2\nmediana (1,675 m)", col = "blue", adj = 0)

# Superior: três rótulos escalonados para cima
lines(y = c(1.93, 1.93), x = c(0.55, 1), col = "blue")
text(x = 0.55, y = 1.93 + 0.02, "Final da hasta superior (1,93 m)",  col = "blue", adj = 0)


points(y = 1.93, x = 1, col = "green", cex = 1.1, lwd = 3)
text(x = 0.65, y = 1.93 - 0.02, "Últ. obs. dentro do LS (1,93 m)", col = "darkgreen", adj = 0)

# Inferior: três rótulos escalonados para baixo
lines(y = c(1.47, 1.47), x = c(0.55, 1), col = "blue")
text(x = 0.55, y = 1.47 - 0.02, "Final da haste inferior (1,47 m)",  col = "blue", adj = 0)

points(y = 1.47, x = 1, col = "green", cex = 1.1, lwd = 3)
text(x = 0.65, y = 1.47 + 0.02, "Últ. obs. dentro do LI (1,47 m)", col = "darkgreen", adj = 0)

# ── Lado DIREITO ──────────────────────────────────────────────────────────────
lines(y = c(ls, ls), x = c(0.9, 1.45), col = "red", lty = 2)
text(x = 1.1, y = ls  + 0.02, "Lim. superior-LS (1,935 m)", col = "red",  adj = 0)

lines(y = c(li, li), x = c(0.9, 1.45), col = "red", lty = 2)
text(x = 1.1, y = li  - 0.02, "Lim. inferior-LI (1,455 m)", col = "red",  adj = 0)

lines(y = c(q1, q1),  x = c(1.0, 1.45), col = "blue")
text(x = 1.45, y = q1 - 0.02, "Prim. quartil-Q1 (1,635 m)",    col = "blue", adj = 1)

lines(y = c(med, med), x = c(1.0, 1.45), col = "blue")
text(x = 1.45, y = med + 0.02, "Média (1,6907 m)", col = "blue", adj = 1)

lines(y = c(q3, q3),  x = c(1.0, 1.45), col = "blue")
text(x = 1.45, y = q3 + 0.02, "Terc. quartil-Q3 (1,755 m)",    col = "blue", adj = 1)

text(x=1, y=2.05, "Outliers", col = "orange", adj = 0)
text(x=1, y=1.38, "Outliers", col = "orange", adj = 0)
