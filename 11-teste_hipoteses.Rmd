...
## Filosofia da ciencia (teoria do conhecimento, epistemologia)
...
```{r ,  out.width='90%', fig.align='center', fig.cap="Flutuação dos valores médios para diversas amostras extraídas de uma mesma população distribuição $\\sim N(\\mu; \\sigma)$", warning = FALSE, echo=TRUE, results='hide'}
N=100
n=140
mu=1600
sigma=120
...
```{r fig70,   out.width='80%', fig.align='center', fig.cap="Regiões críticas, aquém e além das quais, a probabilidade associada aos valores amostrais observados é inferior a $\\frac{\\alpha}{2}$, estabelecendo assim um intervalo com nível de confiança igual a $(1-\\alpha)$", echo=FALSE, warning = FALSE}
...
```{r fig71,   out.width='80%', fig.align='center', fig.cap="Região crítica aquém da qual a probabilidade associada aos valores amostrais observados é inferior a $\\alpha$, estabelecendo assim um intervalo com nível de confiança igual a $(1-\\alpha)$",echo=FALSE,  warning = FALSE}
...
```{r fig72,   out.width='80%', fig.align='center', fig.cap="Região crítica além da qual a probabilidade associada aos valores amostrais observados é inferior a $\\alpha$, estabelecendo assim um intervalo com nível de confiança igual a $(1-\\alpha)$", echo=FALSE, warning = FALSE}
...
```{r fig75,echo=FALSE,    out.width='80%', fig.align='center', fig.cap="Região de rejeição da hipótese nula para o teste unilateral realizado: a região de não rejeição da hipótese nula (região de não significância do teste) está delimitada pelo valor crítico da estatística do teste: $z_{crit} = 1,64$. O valor calculado da estatística ($z_{calc}=-2,50$) situa-se na faixa de não significância do teste, não possibilitando a rejeição da hipótese nula sob aquele nível de confiança", warning = FALSE}
...
```{r fig82, echo=FALSE, echo=FALSE,  out.width='100%', fig.align='center', fig.cap='Visão esquemática das amostras de duas populações'}
...
```{r fig85,echo=FALSE,    out.width='80%', fig.align='center', fig.cap="Região de rejeição da hipótese nula para o teste unilateral realizado: a região de não rejeição da hipótese nula (região de não significância do teste) está delimitada pelo valor crítico da estatística do teste: $z_{crit} = 2,33$. O valor calculado da estatística ($z_{calc}=0,98773$) não nos possibilita a rejeição da hipótese nula sob aquele nível de confiança", warning = FALSE}
...
```{r ,echo=FALSE,   out.width='80%', fig.align='center', fig.cap="O valor calculado da estatística de teste ($F_{calc}=2,56$) situa-se na região não significante do teste, não permitindo a rejeição da hipótese nula de que as variâncias são iguais sob o nível de confiança estabelecido.", warning = FALSE}
...
```{r fig88, echo=FALSE,  out.width='80%', fig.align='center', fig.cap='O valor calculado da estatística de teste ($F_{calc}=2,56$) situa-se na região não significante do teste, não permitindo a rejeição da hipótese nula de que as variâncias são iguais sob o nível de confiança estabelecido.'}
...
```{r , echo=FALSE,   out.width='80%', fig.align='center', fig.cap="Região de rejeição da hipótese nula para o teste uniletaral à direita : a região de não rejeição da hipótese nula (região de não significância do teste) está delimitada pelo valor crítico da estatística do teste: $F_{crit}$ para o nível de significância pretendido ($\\alpha$ em uma cauda)  e ($df_{1}; df_{2}$) graus de liberdade.", warning = FALSE}
...
```{r fig90, echo=FALSE,  out.width='80%', fig.align='center', fig.cap='O valor calculado da estatística de teste ($F_{calc}=3,18$) situa-se na região significante do teste, permitindo a rejeição da hipótese nula de que as variâncias sejam iguais sob o nível de confiança estabelecido.'}
...
- tamanho da amostra $n \ge 30$;  
- probabilidade de sucesso ou fracasso não extremas: $n \cdot \Pi_{0} \ge 5$) e $n \cdot (1-\Pi_{0}) \ge 5$.
...
\\begin{tabular}{|c|c|c|c|c|c|}
\\hline 
\\shortstack{Populações (ou uma segunda variável categórica) \\\\ Variável categórica}  & $B_{1}$  & $B_{2}$   & ... & $B_{s}$ & Total \\\\ 
\\hline 
	$A_{1}$   & $n_{(1,1)}$ & $n_{(1,2)}$ & ... & $n_{(1,s)}$ & $n_{(1,.)}$ \\\\
	$A_{2}$   & $n_{(2,1)}$ & $n_{(2,2)}$ & ... & $n_{(2,s)}$ & $n_{(2,.)}$ \\\\
	...       & ...       & ...       & ... & ...      & ...                \\\\
...
- $n_{(1,.)}$ indica o total (soma) da primeira linha;  
- $n_{(.,1)}$ indica o totla (soma) da primeira coluna; e,    
- $n_{(.,.)}$ indica o total geral (soma) de todas as linhas e colunas da tabela.
...
A estatística ($X$) do Teste Qui-quadrado de homogeneidade (independência) baseia-se na diferença (distância) entre as contagens observados e as contagens esperadas sob a suposição de homogeneidade (independência) pode ser definida da seguinte maneira:
...
```{r fig95, echo=FALSE,   out.width='80%', fig.align='center', fig.cap="Região de rejeição da hipótese nula para o teste unilateral à direita (tipo: maior que): a região de não rejeição da hipótese nula (região de não significância do teste) está delimitada pelo valor crítico da estatística do teste: $x_{crit}$ para o nível de significância pretendido ($\\alpha$ em uma cauda) e ($df$) graus de liberdade.", warning = FALSE}
...
> Exemplo: verifique a independência (homogeneidade) entre a intenção de voto em quatro candidatos distintos e o bairro de residência dos eleitores, considerando moradores de três diferentes bairros, a partir das informações consolidadas na tabela a seguir.
...
As frequências esperadas em cada casela ($i,j$) serão calculadas pela fórmula acima e estão apresentadas na tabela a seguir, em conjunto com as frequências observadas. 
...
$$
X=\sum_{i=1}^4\sum_{j=1}^3 \frac{(O_{(i,j)} - E_{(i,j)})^2}{E_{(i,j)}} = 37,87
$$
...
>Conclusão: face aos dados trazidos à análise rejeitamos a proposição de que a preferência por um determinado candidato não esteja de algum modo associada ao bairro pesquisado sob um nível de significância de 5\% (a probabilidade de cometimento de um erro tipo I). Há alguma associação entre a  preferência por um ou outro candidato e os bairros (Figura \@ref(fig:fig96)).
 

\


> Exemplo: verifique, por meio do teste Qui-quadrado, se há associação entre o hábito tabágico da mãe durante a gravidez e a incidência de baixo peso ao nascer, a partir dos dados de recém-nascidos de Pelotas (RS), 1982.
...
& \cline{2-3}
              Sim & Não        \\ 
\hline              
Fumante & 275 (a) & 2.144 (b) & 2.419 (a+b)   \\
\hline 
Não fumante & 311 (c) & 4.496 (d) & 4.807 (c+d)  \\
\hline 
Total & 586 (a+c) & 6.640 (b+d)  & 7.226   (a+b+c+d)  \\
\hline 
\end{tabular}
\label{tab33}
\end{table}	
...
$$
\begin{cases}
H_{0}: \text{as variáveis são independentes (a flutuação nas contagens é aleatória e não existe associação entre o hábito de fumar e o peso do recém-nascido)} \\
H_{1}: \text{as variáveis não são independentes (há alguma associação entre o hábito de fumar e o peso do recém-nascido)}
\end{cases}
$$
...
Equivale dizer que estamos querendo testar se há independência entre o nascimento com baixo peso e o hábito de fumar da mãe.
...
$$
E_{(1,1)} = \frac{2419 \times 586}{7226}= 196\\
E_{(1,2)} = \frac{2419 \times 6640}{7226}=2223 \\
E_{(2,1)} = \frac{4807 \times 586}{7226}= 390\\
E_{(2,2)} = \frac{4807 \times 6640}{7226}=4417 \\
$$
...
\begin{table}
\centering
\caption{Frequências observadas (esperadas)}
\begin{tabular}{c|c|c|c}
\hline
\multirow{2}{*}{Classificação da mãe} & \multicolumn{2}{c|}{Baixo peso ao nascer} & \multirow{2}{*}{Total}  \\ 
& \cline{2-3}
              Sim & Não        \\ 
\hline              
Fumante & 275 \textbf{(196)} & 2.144 \textbf{(2223)} & 2.419    \\
\hline 
Não fumante & 311 \textbf{(390)} & 4.496 \textbf{(4417)} & 4.807   \\
\hline 
...
$$
X=\sum_{i=1}^r\sum_{j=1}^s \frac{(O_{(i,j)} - E_{(i,j)})^2}{E_{(i,j)}}   \sim \chi^{2}_{((r-1)\times(s-1))} \\
X=\frac{(275-196)^{2}}{196}+\frac{(2144-2223)^{2}}{2223}+\frac{(311-390)^{2}}{390}+\frac{(4496-4417)^{2}}{4417}\\
X=\frac{6241}{196}+\frac{6241}{2223}+\frac{6241}{390}+\frac{6241}{4417}\\
X=31,84+2,81+16,00+1,41=52,06\\
\chi^{2}_{1, 5\%}=3,84 
$$
...
```{r , echo=FALSE,   out.width='80%', fig.align='center', fig.cap="Região de rejeição da hipótese nula para o teste uniletaral à direita (tipo: maior que): a região de não rejeição da hipótese nula (região de não significância do teste) está delimitada pelo valor crítico da estatística do teste: $x_{crit}=3,84$ para o nível de significância pretendido ($\\alpha=0,05$ em uma cauda)  e ($df=1$) grau de liberdade.", warning = FALSE}
...
s <- 2
df <- (r-1)*(s-1)
q_desejado <- qchisq(prob_desejada, df)
d_desejada <- dchisq(q_desejado, df)
q_calculado <- 52.06
d_calculado <- dchisq(q_calculado, df)

ggplot(data.frame(x = c(0, 55)), aes(x)) +
  stat_function(fun = dchisq,
                geom = "line",
                colour = "black",
                args = list(df = df)) +
  ...
  stat_function(fun = dchisq,
                ...
                fill = "red",
                alpha = 0.5,
                xlim = c(q_desejado, 55),
                args = list(df = df)) +
  geom_vline(xintercept = q_desejado, linetype = "dashed", color = "blue", linewidth = 0.5) +
  geom_vline(xintercept = q_calculado, linetype = "dashed", color = "darkgreen", linewidth = 0.5) +
  scale_y_continuous(name = "Densidade") +
  scale_x_continuous(name = "Valores score (X)", breaks = 0:6) +  
  labs(title = "Curva da função densidade \nDistribuição Qui-quadrado", 
       subtitle = paste0("P(0; ", round(q_desejado, 2), ") = (1-\u03b1) em cinza (região de não rejeição) \nP(", 
                        round(q_desejado, 2), "; \U221e) = \u03b1 em vermelho (região de rejeição)")) +
  annotate(geom = "text", x = q_desejado+0.5, y = -0.05, 
           label = paste0("x crítico = ", round(q_desejado, 2)), 
           color = "blue", size = 3.5, hjust = 0.5) +
  annotate(geom = "text", x = q_calculado-0.5, y = -0.05, 
           label = paste0("x calculado = ", q_calculado), 
           color = "darkgreen", size = 3.5, hjust = 0.5) +
  annotate(geom = "text", x = 2, y = 0.3, 
           label = "Região de não rejeição", 
           color = "black", size = 3.5) +
  annotate(geom = "text", x = 5, y = 0.05, 
...
>Conclusão: Os dados analisados nos permitem afirmar, sob um nível de significância de 5\% que não há independência entre o hábito de fumar da mãe durante a gravidez e a incidência de baixo peso ao nascer em recém-nascidos. Ou seja, existe associação estatisticamente significativa entre essas duas variáveis.
...
Como vimos, a aplicação do teste qui-quadrado permite verificar se existe associação entre duas variáveis, com base em um conjunto de observações.

\

A intensidade dessa associação pode ser quantificada por coeficientes que têm por objetivo medir a força da associação entre duas variáveis categorizadas. Um deles é o **coeficiente de contingência de Pearson** ($C$), definido como:
...
Embora $C$ varie entre 0 e 1, seu valor máximo teórico não é 1, mas depende da dimensão da tabela --- especificamente do menor valor entre o número de linhas e de colunas ($k$). Em uma tabela $2 \times 2$, por exemplo, o máximo atingível é aproximadamente 0,707; em uma $3 \times 3$, aproximadamente 0,816. Isso impede a comparação direta de valores de $C$ entre tabelas de dimensões diferentes.

\

Para contornar essa limitação, utiliza-se o **coeficiente de contingência de Pearson modificado** ($C^{*}$), que reescalona $C$ de modo que seu valor máximo seja sempre 1, independentemente da dimensão da tabela.
...
```{r fig97,echo=FALSE,    out.width='80%', fig.align='center', fig.cap="Região de rejeição da hipótese nula para o teste uniletaral à direita (tipo: menor que): a região de não rejeição da hipótese nula (região de não significância do teste) está delimitada pelo valor crítico da estatística do teste: $x_{crit}$ para o nível de significância pretendido ($\\alpha$ em uma cauda)  e ($df$) graus de liberdade.", warning = FALSE}
...
```{r , echo=FALSE, out.width='80%', fig.align='center', fig.cap='Tabela da distribuição Qui-quadrado'}
