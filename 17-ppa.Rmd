...
## Programas de atividade acadêmica para o segundo semestre de 2026
...
### Engenharia Civil: 2STA016 - Estatística e probabilidades (2026/02)
...
>1. Introdução à produção de conhecimento por meio da pesquisa científica: conceitos de população-amostra; parâmetro-estatística;  tipo de variáveis; indexação de dados e operações derivadas; conjuntos, operações e suas representações por meio de diagramas de Venn;  análise exploratória de dados; saneamento de dados (observações com erros não amostrais);  sínteses numéricas de posição; sínteses numéricas de dispersão; medidas separatrizes; apresentações elementares na forma tabular e gráfica para dados qualitativos e quantitativos; sínteses numéricas para dados agrupados em tabelas de frequências.\
>2. Introdução ao cálculo de probabilidades: experimentos aleatórios e determinísticos; espaço amostral, tipos de eventos; conceitos de probabilidade: (1) clássico (a \textit{priori}) ; (2) frequentista (a \textit{posteriori}); (3) conceito axiomático: a probabilidade como uma função; probabilidade da adição de eventos (disjuntos e não disjuntos); probabilidade de eventos (condicionados e independentes);
regra de Bayes; variáveis aleatórias (discretas e contínuas), esperança e variância); funções de probabilidade (massa e densidade);
modelos teóricos de probabilidade para variáveis aleatórias (discretas e contínuas).\
>3. Introdução à inferência estatística: distribuições das médias e proporções amostrais; distribuição das variâncias amostrais; intervalos de confiança para a média, proporção e variância de uma população; intervalos de confiança para a diferença das médias e proporções de duas populações; testes de hipóteses sobre a média e proporção de uma população; testes de hipóteses sobre a diferença das médias e proporções de duas populações.\
>4. Introdução ao planejamento de pesquisas: tipos de pesquisas;
etapas; planejamento do levantamento amostral; tipos de levantamento amostral; dimensionamento de amostras.\ 

#### Cronograma: 

Aula 01 - Apresentações: (a) do programa de atividade acadêmica aprovado; (b) da sala de aula virtual criada (\textit{Google Classroom}); (c) do calendário acadêmico do segundo semestre de 2026 (Resolução CEPE n$^{o}$ 051/2025, \S 1); (d) de aspectos relevantes do Regimento Geral da UEL, especificamente os artigos 60 (\textbf{vedação} ao abono de falta) e 171 (\textbf{deveres} dos membros da Comunidade Universitária: alunos, servidores e professores); (e) da Res. CEPE 017/2023 (amparos). Noções sobre a produção de conhecimento por meio da pesquisa científica. Conceitos introdutórios e terminologia aplicada: população/amostra, parâmetro/estatística,  estatística descritiva/inferencial e variável de estudo, tipos de variáveis.\
Aula 02 - Uso de indexação em operações matemáticas de somas, conjuntos, elementos, subconjuntos e operações elementares, diagramas de Venn, análise exploratória de dados (EDA): dados brutos/saneados (observações com erro não amostral),  estatísticas para dados \textit{qualitativos} (proporções/moda),  sínteses numéricas descritivas para dados \textit{quantitativos} (de \textit{posição}): moda, média.\
Aula 03 - Sínteses numéricas descritivas para dados \textit{quantitativos} (de \textit{variabilidade}): valores mínimo/máximo, amplitude total, variância/desvio padrão, coef. de variação) padronização de dados quantitativos contínuos (Z-\textit{scores}).\
Aula 04 - Sínteses numéricas descritivas para dados \textit{quantitativos} (de separação): percentis, decis e quartis, Sínteses numéricas descritivas para dados \textit{quantitativos} (de forma): assimetria e medida de curtose).\ 
Aula 05 - Apresentação tabular de dados, recomendações gerais para apresentação (elem. obrigatórios), tabelas para dados \textit{qualitativos}: simples e dupla entrada, apresentações gráficas elementares de dados \textit{qualitativos}: colunas, setores.\
Aula 06 - Tabelas para dados \textit{quantitativos}: número de classes, amplitude e limites inf./sup. de cada classe, colunas de frequências absoluta, relativa (simples e acumuladas).\
Aula 07 - Média, moda e variância para dados agrupados em tabelas de frequências.\
Aula 08 - Apresentações gráficas para dados \textit{qualitativos}: barras, setores e para dados \textit{quantitativo}: histograma;\
Aula 09 - Apresentação gráfica de dados \textit{quantitativos}: \textit{box-plot} \
Aula 10 - Primeira avaliação.\
Aula 11 - complementação do calendário acadêmico (cf. Res. 051/2025, art. 1, parág. 3): devolutiva comentada da primeira avaliação.\
Aula 12 - introdução ao cálculo de probabilidades: conceitos essenciais: experimentos aleatórios e determinísticos, a variável aleatória, o conjunto de possíveis resultados do experimento aleatório (espaço amostral e seus elementos), eventos simples e compostos (representações com diagramas de Venn).\
Aula 13 - Introdução ao cálculo de probabilidades: conceitos essenciais de experimentos determinísticos/aleatórios, espaço amostral, eventos e tipos de eventos.\
Aula 14 - progressão dos conceitos de probabilidade: (1) clássico (a \textit{priori}) ; (2) frequentista (a \textit{posteriori}); (3) conceito axiomático: a probabilidade como uma função. \
Aula 15 - probabilidade da união de eventos.\
Aula 16 - probabilidade de eventos condicionados e independentes.\
Aula 17 - introdução ao teorema (regra) de Bayes.\
Aula 18 - variáveis aleatórias discretas: conceito, esperança e variância, função massa de probabilidade.\ 
Aula 19 - variáveis aleatórias contínuas: conceito, esperança e variância, função densidade de probabilidade.\ 
Aula 20 - modelo teórico de Bernoulli e binomial para variáveis aleatórias discretas: esperança, variância e PMF;\
Aula 21: modelo teórico de Poisson para variáveis aleatórias discretas: esperança, variância e PMF;\
Aula 22 - modelos teóricos uniforme e exponencial para variáveis aleatórias contínuas: esperança, variância e PDF.\
Aula 23 - modelo teórico Normal para variáveis aleatórias contínuas: esperança, variância e PDF.\
Aula 24 - segunda avaliação.\
Aula 25 - complementação do calendário acadêmico (cf. Res. 051/2025, art. 1, parág. 3): devolutiva comentada da segunda avaliação.\
Aula 26 - distribuição das médias amostrais de uma população Normal e a construção de intervalos de confiança para $\mu$: (a) $\sigma$ \textit{conhecido} ou $\sigma$ \textit{não conhecido} mas $n$ grande e  (c) $\sigma$ \textit{não conhecido} e $n$ qualquer.\
Aula 27 - distribuição das diferenças amostrais de duas populações Normais e construção de intervalos de confiança para $(\mu_{1}-\mu_{2})$: (a) $\sigma_{1}, \sigma_{2}$ \textit{conhecidos} ou \textit{não conhecidos} mas $n_{1}, n_{2}$ grandes.\ 
Aula 28 - distribuição das diferenças amostrais de duas populações Normais e construção de intervalos de confiança para $(\mu_{1}-\mu_{2})$: (b) $\sigma_{1}, \sigma{2}$ \textit{não conhecidos, mais admitidos iguais} e (c) $\sigma_{1}, \sigma{2}$ \textit{não conhecidos} e quaisquer.\
Aula 29 - distribuição das proporções amostrais de duas populações binomiais e a construção de intervalos de confiança para $\Pi$ por aproximação Normal. \
Aula 30 - distribuição das diferenças das proporções amostrais de duas populações binomiais e a construção de intervalos de confiança para $(\Pi_{1}-\Pi_{2})$ por aproximação Normal. \
Aula 31 - teste de hipóteses sobre a média $\mu$ de uma população e sobre a igualdade das variâncias $\sigma_{1}, \sigma{2}$ de duas populações Normais.\ 
Aula 32 - teste de hipóteses sobre a proporção $\Pi$ de uma população binomial pela aproximação Normal.\
Aula 33 - introdução ao planejamento de levantamentos amostrais, tipos de levantamentos amostrais (probabilísticos e não probabilísticos), levantamento amostral aleatório simples e sistematizado, levantamento amostral aleatório estratificado e por conglomerados.\
Aula 34 - dimensionamento de amostras para inferências sobre médias e proporções.\
Aula 35 - complementação do calendário acadêmico (cf. Res. 051/2025, art. 1, parág. 3)\
Aula 36 - terceira avaliação.\
...
>1. O processo de ensino será composto por um conjunto de atividades presenciais teóricas expositivas e resolução de exercícios em sala de aula a partir do dia 3/08/2026, nos dias e horários determinados no Sistema UEL.\
>2. Todo material utilizado (textos, slides, listas de exercícios) será disponibilizado na sala de aula virtual a ser criada na Plataforma *Google Classroom*, de adesão compulsória por parte do discente, por meio de convite enviado ao seu *email* cadastrado no Sistema UEL.\
>3. Toda comunicação entre discentes e o docente dar-se-á por meio de postagens na plataforma *Google Classroom*.\
>4. As datas das três provas escritas presenciais e do exame final indicadas no cronograma a seguir poderão ser alteradas tanto em razão de **situações não previstas no atual calendário acadêmico quanto do bom progresso das atividades didáticas, mediante comunicação aos alunos com devida antecipação**.\

#### Formas e critérios de avaliação:

Durante o semestre serão realizadas as seguintes atividades avaliativas:\

- três (3) **provas escritas presenciais** (P1, P2, P3) referentes ao conteúdo das aulas e valendo de zero (0) a dez (10) pontos cada uma. A **média final** ($MF$) será calculada pela seguinte expressão (média aritmética simples):\

$$
MF= \frac{1\times(P1) + 1\times(P2) + 1\times(P3)}{3}   
$$\

As **prováveis datas** das três provas escritas presenciais serão: \
    - Turmas 1000/2000: 4/09, 30/10 e 4/12. \
    - Turma 3000: 2/09, 4/11 e 2/12.\  
